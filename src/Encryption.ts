...
class Encryption {
  ...
  protected async *encrypt(input: Blob): AsyncGenerator<ArrayBuffer> {
    ...
    for await (const chunk of this.chunked(input, 0, SLICE_SIZE)) {
      const encryptedChunk = await crypto.subtle.encrypt({ name: ALGORITHM, iv }, this.key, chunk);
  ...
  protected async *decrypt(input: Blob): AsyncGenerator<ArrayBuffer> {
    ...
    for await (const chunk of this.chunked(input, IV_SIZE, SLICE_SIZE + IV_SIZE)) {
  ...
  protected async digest(input: Blob): Promise<string> {
    ...
    for await (const chunk of this.chunked(input, 0, SLICE_SIZE)) {
      hash = await crypto.subtle.digest("SHA-256", this.mergeArrayBuffers(hash, chunk));
    }
  ...
  private async *chunked(blob: Blob, padding: number, chunkSize: number): AsyncGenerator<ArrayBuffer> {
